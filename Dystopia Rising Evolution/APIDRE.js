...
var blackxTemplate = "<table style='background-image:url(https://i.imgur.com/EoXFYBj.png); background-size: 100% 100%; background-repeat: no-repeat; width:250px; height:250px'></table>";

function WODdice(char, player, title, pool1, mod, diff, again, whoSent) {
    var suc = 0;
    var one = 0;
    var rolls = new Array();
    var rr = false;
...
function sendBlackX(char,player){
    var tableCreation = blackxTemplate;
    if (player==1) sendChat("player|"+char, tableCreation);
    else sendChat("character|"+char, tableCreation);
}
...
on("chat:message", function(msg) {
    ...
    if ( cmd[0] == "!apiv20roll" ) {
        ...
        var cmd2 = msg.content.split(';');
            WODdice( msg.playerid, 1, cmd2[1], cmd2[2], cmd2[3], cmd2[4], cmd2[5], inputName );
 
    } else if(cmd[0] == "!apiblackx") {
        sendBlackX(msg.playerid, 1,);
    }
